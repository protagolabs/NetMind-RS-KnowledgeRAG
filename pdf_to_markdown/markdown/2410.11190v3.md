...
Changqiao Wu♠∗ wuchangqiao@inspirai.com  
...
![](https://netmind-public-files.s3.us-west-2.amazonaws.com/e9dcf685a1ce44528d47eeccfc8ff06c/b3db0be682fa3e4ae9484bc2e406d0392b006133e71e73c546b7047ba1265425.jpg)  
...
![](https://netmind-public-files.s3.us-west-2.amazonaws.com/e9dcf685a1ce44528d47eeccfc8ff06c/3b9a7224b827899f1102834d80aec3028488d9af942bb8a8b6b6497d282677d5.jpg)  
...
Language Model - Mini-Omni2 uses the Qwen2-0.5B base version as its foundational language model. We have ported the Llama-based Qwen2 model using the LitGPT[AI, 2023] training framework, employing the configuration of the 0.5B model as the base language model. For the parallel generation of the multi-layer codebook shown in Figure 3, we expanded the vocabulary of the Qwen2 model by adding $7\times4160$ sub-LM-heads, as illustrated in Figure 4, resulting in a vocabulary size of 181,120.  

![](https://netmind-public-files.s3.us-west-2.amazonaws.com/e9dcf685a1ce44528d47eeccfc8ff06c/e99aa7e5267a2968bc814a6d8f199e6681472201e4f4673ab7204e6c73a8c700.jpg)  
Figure 3: Schematic diagram of multi-layer tokens for input and output of the main task model of Mini-Omni2.  

![](https://netmind-public-files.s3.us-west-2.amazonaws.com/e9dcf685a1ce44528d47eeccfc8ff06c/f49e90bb7eb179ac4952f627cb1a94777b9fb1c858e8838e6edc4f3469fce6b7.jpg)  
Figure 4: The special multi-layer vocabulary construction of Mini-Omni2.  

# 3.2 Multimodal Languague Modeling  

Multimodal Modeling - Consider $Y=(y_{i}\in\mathcal{V}_{\mathrm{txt}}\mid i=1,\dots,t_{\mathrm{txt}})$ as a text utterance from a vocabulary $\mathcal{V}_{\mathrm{txt}}$ with length $t_{\mathrm{txt}}$ . The probability of $Y$ can be expressed as $\begin{array}{r}{p(Y)\ =\ \prod_{i=1}^{t_{\mathrm{txt}}}p(y_{i}\ |}\end{array}$ $y_{1},\ldots,y_{i-1})$ . Now, when dealing with a continuous speech signal, we can convert it into discrete speech tokens (dst), represented as $D=(d_{i}\in\mathcal{V}_{\mathrm{dst}}|i=1,\cdot\cdot\cdot,\bar{t}_{\mathrm{dst}})$ using a audio tokenizer. In this context $\nu_{\mathrm{dst}}$ is the vocabulary of discrete speech tokens. These discrete speech tokens can be treated as spoken language within $\nu_{\mathrm{dst}}$ and modeled in a manner similar to text. We combine text and speech in a new vocabulary $\mathcal{V}_{\mathrm{voxt}}$ by $\mathcal{V}_{\mathrm{voxt}}=\mathcal{V}_{\mathrm{txt}}\cup\mathcal{V}_{\mathrm{dst}}$ . Additionally, we introduce visual features $V\in\mathcal{F}_{\mathrm{vis}}$ , where $\mathcal{F}_{\mathrm{vis}}$ represents the continuous features extracted from the image. Therefore, we can model the probability of both speech, text, where $Z=(z_{i}\in\mathcal{V}|i=1,\cdot\cdot\cdot,t)$ . This probability is expressed as $\begin{array}{r}{p(Z)=\prod_{i=1}^{t}p(z_{i}\mid z_{1},\cdots,z_{i-1},V)}\end{array}$ , where $Z$ represents discrete speech tokens $D(\gamma=\mathcal{V}_{\mathrm{dst}})$ , text tokens $Y(\nu=\nu_{\mathrm{txt}})$ , and continuous video features $V(\mathcal{F}=\mathcal{F}_{\mathrm{vis}})$ , or various combinations of $Y,D$ , and $V$ . For the audio and text tokens generated simultaneously, the negative log-likelihood loss can be formulated as in Equation (1).  
...
![](https://netmind-public-files.s3.us-west-2.amazonaws.com/e9dcf685a1ce44528d47eeccfc8ff06c/8e28d9cc3e138305ca4f042e6951c859eac2255ce219a70ca75145b9621b495c.jpg)  
...
| ASR | 1,2,3 | Libritts [Zen et al., 2019] | A1IT1 | 586 h |
| VCTK[datashare,2024] | A1IT1 | 44 h |
| Multilingual LibriSpeech [Pratap et al., 2020] | A1IT1 | 8000h |
| Text QA | 2,3 | Open-Orca [OpenOrca] | T1T2 | 2000K |
| Audio QA | 2,3 | Moss-002-sft-data [Sun et al., 2024] | A1IT1IA2IT2 | 1500K |
| Visual QA | 2,3 | ALLaVA-4V [Sun et al., 2024] | VIA1IT1IA2IT2 | 800K |
| voice QA | final | Alpaca-GPT4[vicgalle,2024] | A1IT1IA2IT2 | 55k |
| Identity finetune [sayan1101, 2024] | A1IT1IA2IT2 | 2k |
| QAassistant [Mihaiii, 2024a] | A1IT1IA2IT2 | 27k |
| Rlhf [Anthropic,2024] | A1IT1IA2IT2 | 367k |
| Trivia-singlechoice [Mihaii, 2024c] | A1IT1IA2IT2 | 17k |
| Trivia-Multichoice [Mihaiii, 2024b] OpenAssistant [OpenAssistan, 2024] | A1IT1IA2IT2 A1IT1IA2IT2 | 20k 2k |
...
![](https://netmind-public-files.s3.us-west-2.amazonaws.com/e9dcf685a1ce44528d47eeccfc8ff06c/ead35648c558081f07d2079ca178d4b1f86e449c71d3495ecc3ee29bc21983dd.jpg)  
...
Google. https://deepmind.google/technologies/gemini/, 2024.   
...
meta. llama3.1, 2024. URL https://llama.meta.com/.  
...
Openai. https://openai.com/index/hello-gpt-4o/, 2024a.  

Openai. https://openai.com/index/gpt-4v-system-card/, 2024b.  
