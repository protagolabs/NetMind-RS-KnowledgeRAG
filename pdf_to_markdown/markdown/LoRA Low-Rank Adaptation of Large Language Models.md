...
![](https://netmind-public-files.s3.us-west-2.amazonaws.com/838c2a275d424531b2bcebf9cf22d25d/ca1d3bd02bdb3a8fd85d2d6ad8d69618318bee5c4f8b78eec27268824c1874cc.jpg)  
...
In the subsequent sections, we propose to use a low-rank representation to encode $\Delta\Phi$ that is both compute- and memory-efficient. When the pre-trained model is GPT-3 175B, the number of trainable parameters $|\Theta|$ can be as small as $0.01\bar{\%}$ of $\left|{\Phi_{0}}\right|$ .  
...
| | | | |
| :--- | :--- | :--- | :--- |
| Batch Size Sequence Length [=&#124; | 32 512 0.5M | 16 256 11M | 1 128 11M |
...
A neural network contains many dense layers which perform matrix multiplication. The weight matrices in these layers typically have full-rank. When adapting to a specific task, Aghajanyan et al. (2020) shows that the pre-trained language models have a low “instrisic dimension” and can still learn efficiently despite a random projection to a smaller subspace. Inspired by this, we hypothesize the updates to the weights also have a low “intrinsic rank” during adaptation. For a pre-trained weight matrix $W_{0}\in\mathbb{R}^{d\times\mathcal{K}}$ , we constrain its update by representing the latter with a low-rank decomposition $W_{0}+\Delta W=W_{0}+B A$ , where $\dot{\boldsymbol{B}}\in\mathbb{R}^{\check{d}\times r},\dot{\boldsymbol{A}}\in\mathbb{R}^{r\times\check{k}}$ , and the rank $r\ll\operatorname*{min}(d,k)$ . During training, $W_{0}$ is frozen and does not receive gradient updates, while $A$ and $B$ contain trainable parameters. Note both $W_{0}$ and $\Delta W=B A$ are multiplied with the same input, and their respective output vectors are summed coordinate-wise. For $h=W_{0}x$ , our modified forward pass yields:  
...
| RoBlarge (AdptP)t | 3.0M | 90.2±.3 96.1±.3 90.2±.7 68.3±1.0 94.8±.2 91.9±.1 83.8±2.9 92.1±.7 |  |  |  |  |  |  |  | 88.4 |
| RoBlarge (AdptP)t | 0.8M |  |  |  |  |  |  | 90.5±.3 96.6±.2 89.7±1.2 67.8±2.5 94.8±.3 91.7±.2 80.1±2.9 91.9±.4 87.9 |  |  |
| RoBlarge (AdptH)t | 6.0M |  |  |  |  |  |  | 89.9±.5 96.2±.3 88.7±2.9 66.5±4.4 94.7±.2 92.1±.1 83.4±1.1 91.0±1.7 87.8 |  |  |
| RoBlarge (AdptH)t | 0.8M |  | 90.3±.3 96.3±.5 87.7±1.7 66.3±2.0 94.7±.2 91.5±.1 72.9±2.9 91.5±.5 |  |  |  |  |  |  | 86.4 |
| RoBlarge (LoRA)t | 0.8M |  | 90.6±.2 96.2±.5 90.2±1.0 68.2±1.9 94.8±.3 91.6±.2 85.2±1.1 92.3±.5 |  |  |  |  |  |  | 88.6 |
...
| GPT-2 M (Adapter)* | 11.09M | 68.9 | 8.71 | 46.1 | 71.3 | 2.47 |
...
| GPT-2 L (Adapter) | 23.00M | 68.9±.3 | 8.70±.04 | 46.1±.1 | 71.3±.2 | 2.45±.02 |
...
![](https://netmind-public-files.s3.us-west-2.amazonaws.com/838c2a275d424531b2bcebf9cf22d25d/219364a0bc83684cb290573573c5fbc2b2d6a3b22b690c790d68a963ae4779c1.jpg)  
...
| Weight Type Rank r | Wq 8 | Wk 8 | W 8 | Wo 8 | Wq,Wk 4 | Wq, W. 4 | Wq, Wk, Wu, W. 2 |
...
| | | | | | | |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
|  | Weight Type | r=1 | =2 | r=4 | r=8 |  = 64 |
| WikiSQL(±0.5%) | Wa. | 68.8 | 69.6 | 70.5 | 70.4 | 70.0 |
| Wq, Wu | 73.4 | 73.3 | 73.7 | 73.8 | 73.5 |
| Wq, Wk, Wu,W. | 74.1 | 73.7 | 74.0 | 74.0 | 73.9 |
| MultiNLI (±0.1%) | Wq. | 90.7 | 90.9 | 91.1 | 90.7 | 90.7 |
| Wq,Wu | 91.3 | 91.4 | 91.3 | 91.6 | 91.4 |
| Wq, Wk, W., W. | 91.2 | 91.7 | 91.7 | 91.5 | 91.4 |
...
![](https://netmind-public-files.s3.us-west-2.amazonaws.com/838c2a275d424531b2bcebf9cf22d25d/35a77c6378f46f95d1cbe7f68f8e9d5e0aaace4d7ae5671c8e04ee6627f447ab.jpg)  
Figure 3: Subspace similarity between column vectors of $A_{r=8}$ and $\scriptstyle A_{r=64}$ for both $\Delta W_{q}$ and $\Delta W_{v}$ . The third and the fourth figures zoom in on the lower-left triangle in the first two figures. The top directions in $r=8$ are included in $r=64$ , and vice versa.  

We make an important observation from Figure 3.  

Directions corresponding to the top singular vector overlap significantly between $A_{r=8}$ and $\scriptstyle A_{r=64}$ , while others do not. Specifically, $\Delta W_{v}$ (resp. $\Delta W_{q})$ ) of $\scriptstyle A_{r=8}$ and $\Delta W_{v}$ (resp. $\Delta W_{q},$ ) of $A_{r=64}$ share a subspace of dimension 1 with normalized similarity $>0.5$ , providing an explanation of why $r=1$ performs quite well in our downstream tasks for GPT-3.  

Since both $A_{r=8}$ and $\scriptstyle A_{r=64}$ are learned using the same pre-trained model, Figure 3 indicates that the top singular-vector directions of $A_{r=8}$ and $\scriptstyle A_{r=64}$ are the most useful, while other directions potentially contain mostly random noises accumulated during training. Hence, the adaptation matrix can indeed have a very low rank.  
...
![](https://netmind-public-files.s3.us-west-2.amazonaws.com/838c2a275d424531b2bcebf9cf22d25d/61ba5abb161a421ea2d1e4d1b0f18d2f67448cf943a3f6ccc696801b2ebc1c73.jpg)  
Figure 4: Left and Middle: Normalized subspace similarity between the column vectors of $\scriptstyle A_{r=64}$ from two random seeds, for both $\Delta W_{q}$ and $\Delta W_{v}$ in the 48-th layer. Right: the same heat-map between the column vectors of two random Gaussian matrices. See Section H.1 for other layers.  
...
$$
\frac{r=4}{\frac{\vert\Delta W_{q}\quad\begin{array}{l l l l l}{\vert\Delta W_{q}\quad}&{W_{q}\quad}&{\mathrm{Random}}\end{array}\vert\begin{array}{l l l l}{\Delta W_{q}\quad}&{W_{q}\quad\mathrm{Random}}\\ {\frac{\vert\vert U^{\top}W_{q}V^{\top}\vert\vert_{F}=\vert\begin{array}{l l l l l l}{0.32}&{21.67}&{0.02}&{\vert\begin{array}{l l l l l}{1.90}&{37.71}&{0.33}&{0.15}\\ {\vert\vert W_{q}\vert\vert_{F}=61.95}\end{array}\vert\begin{array}{l l l l l}{\vert\Delta W_{q}\vert\vert_{F}=6.91}&{\vert}&{\vert\vert\Delta W_{q}\vert\vert_{F}=3.57}&{\quad}\end{array}}\end{array}
$$  
...
![](https://netmind-public-files.s3.us-west-2.amazonaws.com/838c2a275d424531b2bcebf9cf22d25d/cecd4aca07209d7c6297c847b49c65fa8f10b7cec50951f3b0b3c5da4ba59c9f.jpg)  
...
WikiSQL is introduced in Zhong et al. (2017) and contains 56, 355/8, 421 training/validation examples. The task is to generate SQL queries from natural language questions and table schemata. We encode context as $x=\{\mathrm{table~schema,query}\}$ and target as $y=\{\mathrm{SQL}\}$ . The dataset is release under the BSD 3-Clause License.  

SAMSum is introduced in Gliwa et al. (2019) and contains 14, 732/819 training/test examples. It consists of staged chat conversations between two people and corresponding abstractive summaries written by linguists. We encode context as $"\mathrm{\backslashn"}$ concatenated utterances followed by a $\"\backslash\boldsymbol{\mathrm{n}}\backslash\boldsymbol{\mathrm{n}}^{\prime\prime}$ , and target as $y=\{{\mathrm{summary}}\}$ . The dataset is released under the non-commercial licence: Creative Commons BY-NC-ND 4.0.  
...
|  | Optimizer Warmup Ratio LR Schedule Batch Size | 16 16 |  | 16 | AdamW 0.06 Linear | 32 | 16 | 32 | 16 |
| RoBERTa base LoRA | #Earoihs Rate LoRA Config. LoRA α Max Seq. Len. Batch Size | 5304 | 56-04 | 43-04 | 32 4 804 8 512 | 42504 rq=rv =8 | 52504 | 5 804 | 44-04 |
| RoBERTa large LoRA | #Epoihg Rate LoRA Config. LoRA α Max Seq. Len. | 4 3104 128 | 4 41-04 128 | 4 3204 512 | 4 2204 16 128 | 4 2E-04 rq=r=8 512 | 4 3204 512 | 8 4204 512 | 8 2304 512 |
| LoRAt RoBERTa large | Batch Size #Epochs Learning Rate LoRA Config. LoRAα Max Seq. Len. | 10 3E-04 | 10 4E-04 | 20 3E-04 | 4 20 2E-04 | 10 2E-04 16 rq=rv = 8 | 20 3E-04 | 20 4E-04 | 10 2E-04 |
| RoBERTa large AdptP (3M)t | Batch Size #Epochs Learning Rate Bottleneck r Max Seq. Len. | 10 3E-05 | 20 3E-05 | 20 3E-04 | 32 20 3E-04 64 | 128 10 3E-04 128 | 20 3E-04 | 20 3E-04 | 20 3E-04 |
| RoBERTa large AdptP (0.8M)t | Batch Size #Epochs Learning Rate Bottleneck r Max Seq. Len. | 5 3E-04 | 20 3E-04 | 20 3E-04 | 32 20 16 | 10 3E-043E-04 | 20 3E-04 | 20 3E-04 | 20 3E-04 |
| RoBERTa large AdptH (6M)t | Batch Size #Epochs Learning Rate Bottleneck r Max Seq. Len. | 10 3E-05 | 5 3E-04 | 10 3E-04 | 128 32 10 3E-04 64 128 | 5 3E-04 | 20 3E-04 | 20 3E-04 | 10 3E-04 |
| RoBERTa large AdptH (0.8M) | Batch Size #Epochs Learning Rate Bottleneck r Max Seq. Len. | 10 3E-04 | 5 3E-04 | 10 3E-04 | 32 10 3E-043E-04 8 128 | 5 | 20 3E-04 | 20 3E-04 | 10 3E-04 |
...
| DeBERTa XXL LoRA | Batch Size #Epochs Learning Rate | 8 | 8 | 32 | 4 | Linear 6 | 8 | 4 | 4 |
| 5 1E-04 | 16 6E-05 | 30 | 10 | 8 | 11 | 11 | 10 |
| Weight Decay 0 | 0.01 | 2E-04 0.01 | 1E-04 | 1E-04 | 1E-04 0.01 | 2E-04 | 2E-04 |
| 0.15 | 0 | 0 | 0 | 0.01 |  | 0.01 | 0.1 |
| CLS Dropout LoRA Config. |  |  | 0.1 | 0.1 | 0.2 | 0.2 | 0.2 |
| rq =rv=8 |
| LoRAα | Max Seq. Len. | 256 | 128 | 128 | 64 | 8 512 | 320 | 320 | 128 |
...
LoRA+PrefixEmbed $\mathbf{\left(LoRA+PE\right)}$ ) combines LoRA with prefix-embedding tuning, where we insert $l_{p}+l_{i}$ special tokens whose embeddings are treated as trainable parameters. For more on prefixembedding tuning, see Section 5.1.  

LoRA+PrefixLayer $\mathbf{\left(LoRA+PL\right)}$ ) combines LoRA with prefix-layer tuning. We also insert $l_{p}+l_{i}$ special tokens; however, instead of letting the hidden representations of these tokens evolve naturally, we replace them after every Transformer block with an input agnostic vector. Thus, both the embeddings and subsequent Transformer block activations are treated as trainable parameters. For more on prefix-layer tuning, see Section 5.1.  
...
| | | | | | | |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| Hyperparameters | Fine-Tune | PreEmbed | PreLayer AdamW | BitFit | AdapterH | LoRA |
| Optimizer |  |  |  |
| Batch Size | 128 |  |  |
| #Epoch | 2 |  |  |
...
In Table 15, we show the evaluation results of $\mathrm{LoRA+PE}$ and $\mathrm{LoRA+PL}$ on WikiSQL and MultiNLI. First of all, $\mathrm{LoRA+PE}$ significantly outperforms both LoRA and prefix-embedding tuning on WikiSQL, which indicates that LoRA is somewhat orthogonal to prefix-embedding tuning. On MultiNLI, the combination of $\mathrm{LoRA+PE}$ doesn’t perform better than LoRA, possibly because LoRA on its own already achieves performance comparable to the human baseline. Secondly, we notice that $\mathrm{LoRA+PL}$ performs slightly worse than LoRA even with more trainable parameters. We attribute this to the fact that prefix-layer tuning is very sensitive to the choice of learning rate and thus makes the optimization of LoRA weights more difficult in $\mathrm{LoRA+PL}$ .  
...
| | | | | |
| :--- | :--- | :--- | :--- | :--- |
| Method | # Trainable Parameters | BLEU↑ | DART MET个 | TER↓ |
|  | GPT-2 Medium |
| Fine-Tune | 354M | 46.2 | 0.39 | 0.46 |
| Adapter | 0.37M | 42.4 | 0.36 | 0.48 |
...
| :--- | :--- |
| Method | WebNLG |
| BLEU↑ S | A | U | MET↑ S | A | U | TER↓ S | A |
|  |
| Fine-Tune (354M) | 27.7 | 64.2 | 46.5 | GPT-2 Medium .30 | .45 | .38 | .76 | .33 | .53 |
| Adapter (0.37M) | 45.1 | 54.5 | 50.2 | .36 | .39 | .38 | .46 | .40 | .43 |
| Adapter (11M) | 48.3 | 60.4 | 54.9 | .38 | .43 | .41 | .45 | .35 | .39 |
...
| Adapter (0.88M) | 49.8±.0 | 61.1±.0 | 56.0±.0 | .38 | .43 | .41 | .44 | .35 | .39 |
| Adapter (23M) | 49.2±.1 | 64.7±.2 | 57.7±.1 | .39 | .46 | .43 | .46 | .33 | .39 |
...
To evaluate the performance of different adaptation approaches in the low-data regime. we randomly sample 100, 1k and $10\mathrm{k}$ training examples from the full training set of MNLI to form the low-data MNLI- $n$ tasks. In Table 16, we show the performance of different adaptation approaches on MNLI$n$ . To our surprise, PrefixEmbed and PrefixLayer performs very poorly on MNLI-100 dataset, with PrefixEmbed performing only slightly better than random chance $(37.6\%$ vs. $33.3\%$ ). PrefixLayer performs better than PrefixEmbed but is still significantly worse than Fine-Tune or LoRA on MNLI100. The gap between prefix-based approaches and LoRA/Fine-tuning becomes smaller as we increase the number of training examples, which might suggest that prefix-based approaches are not suitable for low-data tasks in GPT-3. LoRA achieves better performance than fine-tuning on both MNLI-100 and MNLI-Full, and comparable results on MNLI-1k and MNLI-10K considering the $(\pm0.3)$ variance due to random seeds.  
...
| PrefixEmbed | lp=32,l= 8 lp=64,li=8 lp=128,li= 8 lp= 256,li = 8 lp= 512,li = 8 | 0.4 M 0.9 M 1.7 M 3.2 M 6.4 M | 55.9 58.7 60.6 63.1 55.9 | 84.9 88.1 88.0 88.6 85.8 |
| PrefixLayer | lp=2,li=2 lp=8,li =0 lp=8,l=8 lp =64,li = 0 lp=32,l=4 | 5.1 M 10.1 M 20.2M 44.1 M 76.1 M | 68.5 69.8 70.1 66.4 64.9 | 89.2 88.2 89.5 89.6 87.9 |
| AdapterH | r=1 r=4 r=8 r=16 r=64 | 7.1 M 21.2 M 40.1 M 77.9 M 304.4 M | 71.9 73.2 73.2 73.2 72.6 | 89.8 91.0 91.5 91.5 91.5 |
| LoRA | r=2 rq=r= 2 rq=rk=ry==1 rq=rv=4 rq=rk=r=ro=2 rq=rv=8 rq=rk=ry=o=64 rq =r=8,lp=8,li = 4 rq=rv = 1 rq=rk =rv =r= 4 rq = rv = 64 | 4.7 M 4.7 M 9.4 M 9.4 M 18.8 M 18.8 M 37.7 M 37.7 M 301.9 M 603.8M | 73.4 73.4 73.3 74.1 73.7 73.7 73.8 74.0 73.6 73.9 | 91.7 91.3 91.4 91.2 91.3 91.7 91.6 91.7 91.4 91.4 |
| LoRA+PE LoRA+PL | rq=rv= 32,lp= 8,li = 4 rq=rv= 64,lp=8,li = 4 rq=rv=8,lp =8,li = 4 | 37.8 M 151.1M 302.1 M 52.8M | 75.0 75.9 76.2 72.9 | 91.4 91.1 91.3 90.2 |
...
| Optimizer Warmup Tokens LR Schedule Batch Size |  | AdamW |
| 250,000 |
| 20 | Linear |
| 20 40 | 100 4 | 128 2 |
| #Epoch Learning Rate | 一 FineTune PrefixEmbed | 40 2.00E-04 | 5.00E-6 4.00E-04 |  |
| 2.00E-04 | 5.00E-04 |
|  | PrefixLayer LoRA | 5.00E-05 | 5.00E-05 5.00E-05 2.00E-4 | 1.00E-04 |
| 16 | 256 |
| Adaptation- Specific | PrefixEmbed lp PrefixEmbed ii | 32 64 |
| PrefixTune | 8 |
| LoRA | lp=li=8 |
|  | rq =rv = 8 |


Table 17: The hyperparameters used for different GPT-3 adaptation methods on $\mathbf{MNLI}(\mathbf{m})–n$ .  
...
![](https://netmind-public-files.s3.us-west-2.amazonaws.com/838c2a275d424531b2bcebf9cf22d25d/d11061af6898ee44b50c616b4475455e54ab37177076fd7f6f7288abd2de586b.jpg)  
Figure 6: Normalized subspace similarity between the column vectors of $A_{r=8}$ and $\scriptstyle A_{r=64}$ for both $\Delta W_{q}$ and $\Delta W_{v}$ from the 1st, 32nd, 64th, and 96th layers in a 96-layer Transformer.  
...
![](https://netmind-public-files.s3.us-west-2.amazonaws.com/838c2a275d424531b2bcebf9cf22d25d/6067947c93ed20b8b4a41b75e4aecd75a697f6344026a1b73232e17823c2a903.jpg)  
Figure 7: Normalized subspace similarity between the column vectors of $\scriptstyle A_{r=64}$ from two randomly seeded runs, for both $\Delta W_{q}$ and $\Delta W_{v}$ from the 1st, 32nd, 64th, and 96th layers in a 96-layer Transformer.  
...
![](https://netmind-public-files.s3.us-west-2.amazonaws.com/838c2a275d424531b2bcebf9cf22d25d/aae9f55e2fce67587c8f4246ce9a7452489f09be42e06f47c14a68928ad493fa.jpg)  
